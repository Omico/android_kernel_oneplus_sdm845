...
static int smblib_otg_toggle_vote_callback(struct votable *votable, void *data,
...
int op_set_prop_otg_switch(struct smb_charger *chg, bool enable)
{
	int rc = 0;
	u8 power_role;
	u8 ctrl = 0;
	...
	for (i = 0; i < 10; i++) {
		...
		if (rc < 0) {
			smblib_err(chg,
				   "Couldn't write 0x%02x to 0x1368 rc=%d\n",
				   power_role, rc);
			return rc;
		}
...
char dump_val[2048];
static inline void op_dump_reg(struct smb_charger *chip, u16 addr_start,
			       u16 addr_end)
{
	u8 reg = 0;
	u16 addr;
	...
	for (addr = addr_start; addr <= addr_end; addr++) {
		...
		scnprintf(reg_val, sizeof(reg_val), "%x=%0x;", addr, reg);
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	vote(chg->usb_icl_votable, DCP_VOTER, true, current_limit_ua);
	...
	pr_info("apsd result=0x%x, name=%s, psy_type=%d\n", apsd_result->bit,
		apsd_result->name, apsd_result->pst);
	...
	set_property_on_fg(chg, POWER_SUPPLY_PROP_SET_ALLOW_READ_EXTERN_FG_IIC,
			   true);
...
static void smblib_handle_typec_removal(struct smb_charger *chg)
{
	...
	/* enable DRP */
	val.intval = chg->otg_switch ? POWER_SUPPLY_TYPEC_PR_DUAL
...
static void op_get_aicl_work(struct work_struct *work)
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, get_aicl_work);
	int rc, settled_ua;
...
static void op_dash_check_work(struct work_struct *work)
{
	...
	struct smb_charger *chg =
		container_of(dwork, struct smb_charger, dash_check_work);
	int soc, temp;
...
static void op_connect_temp_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg =
		container_of(dwork, struct smb_charger, connecter_check_work);
	if (!chg->vbus_present)
		return;
...
static void op_handle_usb_removal(struct smb_charger *chg)
{
	...
	chg->dash_check_count = 0;
	vote(chg->fcc_votable, DEFAULT_VOTER, true, SDP_CURRENT_UA);
...
static int op_set_collapse_fet(struct smb_charger *chg, bool on)
{
	...
	if (rc < 0) {
		smblib_err(chg, "Couldn't write %s to 0x%x rc=%d\n",
			   on ? "on" : "off", USBIN_5V_AICL_THRESHOLD_CFG_REG,
			   rc);
		return rc;
	}
	...
	rc = smblib_masked_write(
		chg, USBIN_AICL_OPTIONS_CFG_REG,
...
static void set_usb_switch(struct smb_charger *chg, bool enable)
{
	...
	if (enable) {
		...
		if (chg->boot_usb_present && chg->re_trigr_dash_done) {
			vote(chg->usb_icl_votable, AICL_RERUN_VOTER, true, 0);
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, re_kick_work.work);
	if (chg->vbus_present) {
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, rechk_sw_dsh_work.work);
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, op_check_apsd_work.work);
	union power_supply_propval vbus_val;
	int rc;
	const struct apsd_result *apsd_result;
	...
	if (chg->recovery_boost_count >= BOOST_BACK_COUNT
		...
		if (is_usb_present(chg)) {
			...
			if (effect_result > DEFAULT_AGAING_CHG_MA * 1000) {
				vote(chg->usb_icl_votable, DCP_VOTER, true,
				     (effect_result - USBIN_150MA));
...
static void op_check_allow_switch_dash_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg =
		container_of(dwork, struct smb_charger, check_switch_dash_work);
	const struct apsd_result *apsd_result;
...
static void op_otg_icl_contrl(struct smb_charger *chg)
{
	...
	} else if (cap > chg->OTG_LOW_BAT && icl_pre == chg->OTG_LOW_BAT_ICL) {
...
static void op_check_charge_timeout(struct smb_charger *chg)
{
	...
	if (chg->vbus_present && batt_status == POWER_SUPPLY_STATUS_CHARGING)
		count++;
	else
		count = 0;
...
static int get_prop_fg_capacity(struct smb_charger *chg)
{
	...
	rc = get_property_from_fg(chg, POWER_SUPPLY_PROP_FG_CAPACITY,
				  &capacity);
	if (rc) {
		pr_err("Couldn't get capacity rc=%d\n", rc);
		capacity = DEFAULT_BATT_CAPACITY;
...
void set_chg_ibat_vbat_max(struct smb_charger *chg, int ibat, int vfloat)
{
	...
	vote(chg->fcc_votable, DEFAULT_VOTER, true, ibat * 1000);
	vote(chg->fv_votable, DEFAULT_VOTER, true, vfloat * 1000);
...
static int handle_batt_temp_little_cold(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if (temp_region != BATT_TEMP_LITTLE_COLD || chg->is_power_changed
	    || chg->recharge_pending) {
		pr_err("triggered\n");
		...
		set_chg_ibat_vbat_max(chg, chg->ibatmax[BATT_TEMP_LITTLE_COLD],
				      chg->vbatmax[BATT_TEMP_LITTLE_COLD]);
...
static int handle_batt_temp_cool(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if (temp_region != BATT_TEMP_COOL || chg->is_power_changed
	    || chg->recharge_pending) {
		pr_err("triggered\n");
		chg->recharge_pending = false;
		chg->is_power_changed = false;
		...
		set_chg_ibat_vbat_max(chg, chg->ibatmax[BATT_TEMP_COOL],
...
static int handle_batt_temp_little_cool(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if (temp_region != BATT_TEMP_LITTLE_COOL || chg->is_power_changed
	    || chg->recharge_pending) {
		pr_err("triggered\n");
		chg->recharge_pending = false;
		chg->is_power_changed = false;
		...
		if (vbat_mv > 4180) {
			set_chg_ibat_vbat_max(
				chg, 450, chg->vbatmax[BATT_TEMP_LITTLE_COOL]);
			chg->temp_littel_cool_set_current = false;
		} else {
			set_chg_ibat_vbat_max(
				chg, chg->ibatmax[BATT_TEMP_LITTLE_COOL],
...
static int handle_batt_temp_prenormal(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if (temp_region != BATT_TEMP_PRE_NORMAL || chg->is_power_changed
	    || chg->recharge_pending) {
		pr_err("triggered\n");
		chg->recharge_pending = false;
		chg->is_power_changed = false;
		...
		set_chg_ibat_vbat_max(chg, chg->ibatmax[BATT_TEMP_PRE_NORMAL],
...
static int handle_batt_temp_normal(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if ((temp_region != BATT_TEMP_NORMAL) || chg->is_power_changed
	    || chg->recharge_pending) {
		pr_err("triggered\n");
		chg->recharge_pending = false;
		chg->is_power_changed = false;
		...
		set_chg_ibat_vbat_max(chg, chg->ibatmax[BATT_TEMP_NORMAL],
...
static int handle_batt_temp_warm(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if ((temp_region != BATT_TEMP_WARM) || chg->is_power_changed
	    || chg->recharge_pending) {
		pr_err("triggered\n");
		chg->is_power_changed = false;
		chg->recharge_pending = false;
		...
		if (temp_region == BATT_TEMP_HOT
		    || temp_region == BATT_TEMP_COLD)
			op_charging_en(chg, true);
		op_temp_region_charging_en(chg, chg->vbatmax[BATT_TEMP_WARM]);
		set_chg_ibat_vbat_max(chg, chg->ibatmax[BATT_TEMP_WARM],
...
static int handle_batt_temp_hot(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if ((temp_region != BATT_TEMP_HOT) || chg->is_power_changed) {
		pr_err("triggered\n");
		chg->is_power_changed = false;
		op_charging_en(chg, false);
		...
		chg->mBattTempBoundT4 = chg->BATT_TEMP_T4;
		chg->mBattTempBoundT5 = chg->BATT_TEMP_T5;
		chg->mBattTempBoundT6 = chg->BATT_TEMP_T6 - BATT_TEMP_HYST;
		/* from hot to warm */
...
static void op_check_charger_collapse(struct smb_charger *chg)
{
	...
	pr_debug("chger_stat=0x%x, aicl_stats =0x%x, chg_en =%d\n", chger_stat,
		 pwer_source_stats, chg_en);
	curr = get_prop_batt_current_now(chg) / 1000;
	stat = !chg->chg_done && !is_usb_supend && (curr > 20) && chg_en
...
void checkout_term_current(struct smb_charger *chg)
{
	...
	if (chg_full) {
		...
		pr_info("chg_done:CAP=%d (Q:%d),VBAT=%d (Q:%d),IBAT=%d (Q:%d),BAT_TEMP=%d\n",
			get_prop_batt_capacity(chg), get_prop_fg_capacity(chg),
			get_prop_batt_voltage_now(chg) / 1000,
			get_prop_fg_voltage_now(chg) / 1000,
			get_prop_batt_current_now(chg) / 1000,
			get_prop_fg_current_now(chg) / 1000,
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg = container_of(dwork, struct smb_charger,
					       non_standard_charger_check_work);
	...
	if (chg->non_stand_chg_count >= NON_STANDARD_CHARGER_CHECK_S) {
		apsd_result = smblib_update_usb_type(chg);
		if (apsd_result->bit == DCP_CHARGER_BIT
		    || apsd_result->bit == OCP_CHARGER_BIT)
			return;
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, re_det_work.work);
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, op_re_set_work.work);
	int rc = 0;
...
static void op_heartbeat_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg =
		container_of(dwork, struct smb_charger, heartbeat_work);
	enum temp_region_type temp_region;
	...
	if (temp_region == BATT_TEMP_LITTLE_COOL) {
		if (vbat_mv > 4180 + 20 && chg->temp_littel_cool_set_current) {
			chg->is_power_changed = true;
	...
	if (!chg->chg_ovp && chg->chg_done && temp_region > BATT_TEMP_COLD
...
out:
	smblib_dbg(
		chg, PR_OP_DEBUG,
		"CAP=%d (Q:%d), VBAT=%d (Q:%d), IBAT=%d (Q:%d), BAT_TEMP=%d, CHG_TYPE=%d, VBUS=%d\n",
		get_prop_batt_capacity(chg), get_prop_fg_capacity(chg),
		get_prop_batt_voltage_now(chg) / 1000,
		get_prop_fg_voltage_now(chg) / 1000,
		get_prop_batt_current_now(chg) / 1000,
		get_prop_fg_current_now(chg) / 1000, get_prop_batt_temp(chg),
		chg->usb_psy_desc.type, vbus_val.intval);
	/*update time 6s*/
...
void op_write_backup_flag(struct smb_charger *chg, bool bk_flag)
{
	...
	rc = op_masked_write(chg, SOC_DATA_REG_0, BIT(7), bk_flag ? BIT(7) : 0);
	if (rc) {
...
static void clear_backup_soc(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(
		...
		(BIT(7) & invalid_soc) | (BIT(6) & invalid_soc)
			| (BIT(5) & invalid_soc) | (BIT(4) & invalid_soc)
			| (BIT(3) & invalid_soc) | (BIT(2) & invalid_soc)
			| (BIT(1) & invalid_soc));
	if (rc)
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->heartbeat_work, op_heartbeat_work);
